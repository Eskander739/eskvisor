from backend import User, UserRole, RoleBasedAccessControl
...
def example_usage():
    ...
    print(f"Виртуализатор может создать ВМ: " f"{virtualizator.can_manage_vm()}")

    # Проверка может ли оператор перезагрузить ВМ
    print(
        f"Оператор может перезагрузить ВМ: "
        f"{operator.has_permission('virtual_machine', 'start')}"
    )

    # Проверка может ли инфра-админ управлять сетями
    print(
        f"Инфра-админ может управлять сетями: " f"{infra_admin.can_manage_networks()}"
    )

    # Проверка может ли супер-админ управлять пользователями
    print(
        f"Супер-админ может управлять пользователями: "
    ...
    print(
        f"Инфра-админ может назначить супер-админа: "
        f"{RoleBasedAccessControl.can_assign_role(UserRole.INFRA_ADMIN, UserRole.SUPER_ADMIN)}"
    )

    print(
        f"Виртуализатор может назначить оператора: "
