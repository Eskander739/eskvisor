...
from agent.client.api.dependencies import get_queue_manager_service, get_vm_live_monitor_service, get_ws_handler, \
    get_task_dispatcher_service, get_vnc_manager
        ...
        while True:
            ...
            try:
                ...
                if object == "system":
                    ...
                    await websocket.send_json(DefaultMessage(code=CommandMessagesEnum.not_implemented_error.name,
                                                             success=False,
...
async def vnc(websocket: WebSocket, vm_name: str,
    ...
    await websocket.accept()
    active_connections.append(websocket)
    logger.info(f"Новое VNC WebSocket подключение для VM '{vm_name}'. Всего: {len(active_connections)}")
    ...
    try:
        ...
        vnc_connect_info = VNCConnectInfo(code=CommandMessagesEnum.vnc_connection_successfully_created.name,
                                          vm_name=vm_name,
                                          vnc_port=vnc_port,
                                          ws_port=ws_port,
        ...
        while True:
            ...
            try:
                ...
                if action == "get_status":
                    ...
                    await websocket.send_json(VNCStatusInfo(code=CommandMessagesEnum.vnc_status_successfully_readed.name,
                                                            vm_name=vm_name,
                    ...
                    ws_port = vnc_manager.start_vnc_proxy(vm_name, vnc_port)
                    await websocket.send_json(VNCRestartProxy(code=CommandMessagesEnum.vnc_proxy_successfully_restarted.name,
                                                              vm_name=vm_name,
                                                              new_ws_port=ws_port,
