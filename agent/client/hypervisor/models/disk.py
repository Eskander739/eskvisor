...
class Disk(BaseModel):
    ...
    capacity_gb: float | None = Field(None, description="Размер диска в гигабайтах", ge=0.0)
    allocation_bytes: int | None = Field(None, description="Фактически занято байт", ge=0)
    allocation_gb: float | None = Field(None, description="Фактически занято гигабайт", ge=0.0)
    ...
    # Опциональные поля
    uuid: str | None = Field(None, description="UUID диска",
    ...
    permissions: str | None = Field(None, description="Права доступа в восьмеричном формате", pattern=r"^0[0-7]{3}$")
    cluster_size: int | None = Field(None, description="Размер кластера в байтах", ge=512)
    ...
    refcount_bits: int | None = Field(None, description="Бит счетчика ссылок", ge=1, le=64)
    ...
    def supports_snapshots(self) -> bool:
        ...
        return self.format in {DiskFormat.QCOW2, DiskFormat.QCOW, DiskFormat.VHDX, DiskFormat.VDI}
