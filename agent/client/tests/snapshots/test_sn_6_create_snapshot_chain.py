...
@pytest.mark.tags("SN‑06", "Создание цепочки снапшотов (несколько последовательных)")
def test_sn_06_create_snapshot_chain(
    snapshot_session, create_stopped_vm_func, vm_session, storage_session
):
    """
    SN‑06: Создание цепочки снапшотов (несколько последовательных)

    Создать несколько снапшотов один за другим. Проверить, что дерево снапшотов отражает правильную последовательность.
    """
