...
def test_sn_05_clone_vm_from_snapshot(
    snapshot_session, create_running_vm_func, vm_session, storage_session, vm_state
):
    """
    ...
    try:
        ...
        # ________________________________Клонирование ВМ из снапшота_____________
        revert_vm_info = snapshot_session.clone_vm_from_snapshot(
            SnapshotCloneRequest(
        ...
        disks_cloned_vm = sorted(storage_session.get_disks_by_vm(new_vm_name))
        ...
        if cloned_vm_created:
            delete_vm_info = vm_session.delete_vm_with_force(name=new_vm_name)
            assert (
