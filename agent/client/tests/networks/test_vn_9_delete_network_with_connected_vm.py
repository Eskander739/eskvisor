...
def test_vn_09_delete_network_with_connected_vm(
    ...
    try:
        ...
        delete_network_info = network_session.delete_network(nat_params.name, True)
        assert (
            delete_network_info.code
            == CommandMessagesEnum.virtual_network_have_connected_vms.name
        )
    finally:
